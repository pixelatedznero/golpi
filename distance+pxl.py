...
def evaluate(individual, device):
    ...
    # Evaluate pixels per frame
    pixels = ch.pixelsperframe()
    percent_quality = 100 - pixels
    if percent_quality < 0:
        percent_quality = 0
    maxdistance = iterations/2
    divisor = 100 / maxdistance
    pxl_quality_inrelation = percent_quality/divisor
    if pixels < 5:
        pxl_quality_inrelation = -10

    fitness_score = distances[max(distances)] + pxl_quality_inrelation
